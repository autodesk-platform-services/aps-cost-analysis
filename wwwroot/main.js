...
viewer.addEventListener(Autodesk.Viewing.GEOMETRY_LOADED_EVENT, async function () {
    ...
    async function update() {
        const allMaterials = await fetch("/materials").then(resp => resp.json());
        let allMaterialNames = allMaterials.map(m => m.material);
        const dbids = getAllDbIds(viewer);
        const allProperties = await getProperties(viewer, dbids, materialProperty);
        let modelMaterialNames = allProperties.map(p => p.properties[0].displayValue);
        const sharedMaterials = allMaterialNames.filter((element) => modelMaterialNames.includes(element));
    ...
    function onMaterialSelected(material) {
        ...
        }, function () { }, [], { includeInherited: true });
    }
...
async function calculateCostBreakdown(viewer, materials, materialProperty, currencies) {
    const summary = [];
    let totalCost = 0;
    for (const material of materials) {
        ...
        const units = count;
        const current_currency = currencies.find(currency => currency.currency == material.currency);
